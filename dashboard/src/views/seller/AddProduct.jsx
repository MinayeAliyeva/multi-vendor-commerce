...
const AddProduct = () => {
    ...
    // Product form input-larinin hamisi bir state object icinde saxlanilir.
    ...
    const inputHandle = (e) => {
        // input name="price" olsa state.price, name="brand" olsa state.brand yenilenir.
        setState({
            ...state,
    ...
    // category secilen deyeri, allCategory ise dropdown-da gosterilen filterlenmis listi saxlayir.
    ...
    const categorySearch = (e) => {
        ...
        if (value) {
            // Search yazildiqca category listi local array-den filter olunur.
    ...
    <form>
        ...
        <div className='flex flex-col mb-3 md:flex-row gap-4 w-full text-[#d0d2d6]'>
	            <div className='flex flex-col w-full gap-1 relative'>
	                ...
	                {/* readOnly input dropdown kimi istifade olunur; klikde category paneli acilir. */}
                    ...
                    <div className='flex justify-start items-start flex-col h-[200px] overflow-x-scrool'>
	                        {
	                            allCategory.map((c,i) => <span className={`px-4 py-2 hover:bg-indigo-500 hover:text-white hover:shadow-lg w-full cursor-pointer ${category === c.name && 'bg-indigo-500'}`} onClick={()=> {
	                                // Category secilende dropdown baglanir ve search reset olunur.
	                                setCateShow(false)
	                                setCategory(c.name)
                                setSearchValue('')
                                setAllCategory(categorys)
